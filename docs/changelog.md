...
- **Passkeys (WebAuthn)** — register/list/delete credentials; passwordless login via `/login/webauthn/*`. Passkeys are not an MFA method after password.
- **Docs MFA** — docs sign-in prompts for a TOTP/backup code via `POST /docs/session/mfa` when the account has MFA enabled.
- **Docs passkey login** — passwordless **Login with Passkey** on the docs sign-in page and re-auth modal (`POST /docs/session/webauthn/options` + `/verify`).
...
- **MFA / WebAuthn config** — `mfa.*` and `webauthn.*` (and matching env vars) for TOTP issuer, challenge TTL, RP name/ID, and allowed origins. See [Configuration](configuration.md#mfa-and-passkeys).

### Removed

- **HTTP Basic Auth for API requests** — `Authorization: Basic …` is rejected. Use `Authorization: Bearer <token>` or `X-API-Key`. Password login still accepts email/password in the request body (`POST /login`, docs session).
